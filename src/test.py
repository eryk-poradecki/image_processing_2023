...
def generate_mask_angles(r1: int, r2: int, a1: int, a2: int, N: int = N) -> np.ndarray:
    ...
    image_array[mask2] = 255
    image_array[mask1] = 0
    
    return image_array

def generate_mask_angles_3d(r1: int, r2: int, a1: int, a2: int, N: int = N, channels: int = 1) -> np.ndarray:
    assert r1 < r2

    image_array = np.zeros((N, N, channels), dtype=np.uint8)
    center = (N // 2, N // 2)
    ...
    mask = (r <= r2) & (a1 <= theta) & (theta <= a2)  # Create a single-channel mask

    for i in range(channels):
        image_array[:, :, i] = mask.astype(np.uint8)

    image_array[:, :, 0] -= (r <= r1) & (a1 <= theta) & (theta <= a2)
...
# image_array = generate_mask_angles(30, 255, np.radians(80), np.radians(100))
# mirrored_image_array = np.flipud(np.fliplr(image_array))
# sum_result = image_array + mirrored_image_array
